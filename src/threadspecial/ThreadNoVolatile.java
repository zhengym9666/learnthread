...
class ThreadNoVol implements Runnable {
...
//解决:使用volatile关键字修饰共用变量，使得每次取该变量的值都是从主内存中去取。
    public class ThreadNoVolatile{
        public static void main(String[] args) throws InterruptedException {
            ThreadNoVol noVolatile = new ThreadNoVol();
            Thread thread1 = new Thread(noVolatile);
            thread1.start();
            Thread.sleep(3000);
            noVolatile.setRunning(false);//设置为false之后，子线程仍然在执行
